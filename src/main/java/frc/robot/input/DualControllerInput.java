// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.input;

import edu.wpi.first.wpilibj.XboxController;
import frc.robot.Constants;
import frc.robot.math.Vec2;
import frc.robot.util.MoPrefs;
import frc.robot.util.MoPrefs.Pref;
import frc.robot.util.MoUtils;

/** Add your docs here. */
public class DualControllerInput implements MoInput {
    private final XboxController primaryController;
    private final XboxController secondaryController;

    private Pref<Double> driveDeadzone = MoPrefs.driveDeadzone;
    private Pref<Double> driveCurve = MoPrefs.driveCurve;

    public DualControllerInput(Constants.HIDPort primaryPort, Constants.HIDPort secondaryPort) {
        this.primaryController = new XboxController(primaryPort.port);
        this.secondaryController = new XboxController(secondaryPort.port);
    }

    private double applyDriveInputTransforms(double value) {
        return MoUtils.curve(MoUtils.deadzone(value, driveDeadzone.get()), driveCurve.get());
    }

    @Override
    public Vec2 getMoveRequest() {
        return new Vec2(primaryController.getLeftX(), primaryController.getLeftY())
                .scalarOp(this::applyDriveInputTransforms);
    }

    @Override
    public double getTurnRequest() {
        return -1 * applyDriveInputTransforms(primaryController.getRightX());
    }

    @Override
    public boolean getShouldUseSlowSpeed() {
        return primaryController.getLeftBumper();
    }

    @Override
    public boolean getReZeroGyro() {
        return primaryController.getStartButton();
    }
}
